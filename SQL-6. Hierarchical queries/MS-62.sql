...
Regiment #1271A	1
   First Company	1
      ...
      Platoon #2	1
...					...
Дополнительное условие - НЕ выводить те подразделения (а также подчиненные им подразделения), 
которые не имеют в составе ни одного военнослужащего
*/
...
and a.name = ALL (select f.name from military_units f
where (SELECT COUNT(person_id) FROM staff WHERE unit_id=a.unit_id ) = 0 )